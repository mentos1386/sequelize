...
class ConnectionManager extends AbstractConnectionManager {
  ...
  connect(config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // oids for hstore, geometry, geography and ltree are dynamic - so select them at connection time
      ...
      if (dataTypes.HSTORE.types.postgres.oids.length === 0 && supportedVersion) {
        query += 'SELECT typname, oid, typarray FROM pg_type WHERE typtype = \'b\' AND typname IN (\'hstore\', \'geometry\', \'geography\', \'ltree\')';
      }
      ...
      return new Promise((resolve, reject) => {
        connection.query(query)
          ...
          .on('row', row => {
            ...
            } else if (row.typname === 'ltree'){
              type = dataTypes.postgres.LTREE;
            }
