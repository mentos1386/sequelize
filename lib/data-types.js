...
function LTREE() {
  if (!(this instanceof LTREE)) return new LTREE();
}
inherits(LTREE, ABSTRACT);

LTREE.prototype.key = LTREE.key = 'LTREE';
LTREE.prototype.toSql = function toSql() {
  return 'LTREE';
};
LTREE.prototype.validate = function validate(value) {
  if (!Validator.matches(String(value), /^[^.]+(?:[.][^.]+)*$/)) {
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid LTREE', value));
  }
 ...
 * @property {function} LTREE Representing labels of data stored in a hierarchical tree-like structure, Only avalable in postgres.
